...
def predict_single_image(img_path, model_rpn, model_classifier_only, cfg, class_mapping):
    ...
    for cls_num, box in boxes.items():
        ...
        print(class_mapping[cls_num] + ":" + str(cls_num))
        for b in boxes_nms:
...
def load_model():
    ...
    model_rpn.compile(optimizer='sgd', loss='mse')
    model_classifier.compile(optimizer='sgd', loss='mse')
    return model_rpn,model_classifier_only,cfg

def predict_one_image(model_rpn,model_classifier,cfg,image):
    X, ratio = format_img(image, cfg)
    if K.image_dim_ordering() == 'tf':
        X = np.transpose(X, (0, 2, 3, 1))
    # get the feature maps and output from the RPN
    [Y1, Y2, F] = model_rpn.predict(X)
    ...
    for cls_num, box in boxes.items():
        ...
        for b in boxes_nms:
            ...
            # print('{} prob: {}'.format(b[0: 4], b[-1]))
            cls.append(cls_num)
            box_real.append([b[0],b[1],b[2],b[3]])
            prop.append(b[-1])
    return cls,box_real,prop


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--path', '-p', default='/home/zhang/桌面/word_detect/jinnan2_round1_train_20190305/restricted/190119_184116_00166918.jpg', help='image path')
...
if __name__ == '__main__':
    ...
    # image = cv2.imread('/home/zhang/桌面/word_detect/jinnan2_round1_train_20190305/restricted/190119_184116_00166918.jpg')
    # cls,box_real,prop = predict_one_image(model_rpn,model_classifier,cfg,image)
    # # print(cls,box_real,prop)
    # for i,c in enumerate(cls):
    #     x1,y1,w,h = box_real[i]
    #     cv2.rectangle(image,(int(x1),int(y1)),(int(w),int(h)),(255,0,0),2)
    #     cv2.putText(image,"{} {}".format(c,prop[i]),(int(x1),int(y1+1)),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0), 1)
    # cv2.imshow("im",image)
    # cv2.waitKey(0)